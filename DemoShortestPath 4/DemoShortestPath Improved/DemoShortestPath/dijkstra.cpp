...
void releaseMinHeap(MinHeap *minHeap)
{
	if (minHeap != NULL)
	{
		if (minHeap->pos != NULL)
...
void releaseGraph(Graph* graph)
{
	...
	for (int i = 0; i < graph->V; ++i)
	{
		...
		AdjListNode *travelNode = graph->vertices[i].head;
...
struct MinHeap* createMinHeap(int capacity)
{
	...
	minHeap->pos = (int *) new int[capacity];
	minHeap->size = 0;
...
void dijkstra(struct Graph* graph, int src, int dest, vector<int> & shortestPath)
{
	...
	shortestPath.insert (it , dest);
	int idx = graph->vertices[dest].backtrack;
	while (idx != -1)
	{
		it = shortestPath.begin();
		shortestPath.insert (it , idx);
...
void findShortestChannel(struct Graph* graph, int src, int dest, vector<int> & shortestChannel)
{
	dijkstra(graph, src, dest, shortestChannel);
}